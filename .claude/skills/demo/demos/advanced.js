...
// AI CLI update (skip countdown + spinner), agent auto-start into claude, then
// exiting twice and the auto-shutdown flow.
...
// is stopped, so no state is shown - the header is just version and workspace
...
// 4. Resume container (src/commands/dev.ts)
out(`${BLUE}\u25cf${R}  Resuming dev container...${NL}`, 1.0);

// 5. AI CLI update (templates/startup.mjs, summary mode): blank separator line,
...
// 6. Sandbox welcome (same as quickstart)
out(
...
// 7. Auto-start launches the configured agent - no typed command (generated .bashrc,
// src/lib/dockerfile-builder.ts)
out(`${GREEN}\u25cf${R}  ${GREY}Auto-start enabled: launching ${ORANGE}claude${GREY}.${R}${NL}${NL}`, 1.1);

// 8. Claude Code header, input box, status line (same as quickstart)
out(
...
// 9. Type /exit in the input box: cursor up 3 rows to the input line, column 4
// (over the resting block cursor), type, then move back below the status line.
out(`\u001b[3A\r\u001b[3C`, 0.2);
type("/exit", 0.4);
out(`\u001b[3B\r${NL}`, 0.4);

// 10. Back in the container shell (the real PS1 from the generated .bashrc), exit to host
...
// 11. Stop-container confirm, then collapsed with the answer (cursor up 3, clear down)
out(
...
// 12. Container stops; totopo prints a trailing blank line and exits to the host prompt
