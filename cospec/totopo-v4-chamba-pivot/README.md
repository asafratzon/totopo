...
- 2026-08-23: post-checkpoint (user): both demo GIFs re-rendered in the container after all - the demos are synthetic, so no Docker is involved - showing `totopo v4.0.0` and, in the advanced one, no host audio server; FR-25 and the phase-4 record corrected accordingly, since the plan had listed this as a host step. The README now says why voice went (Claude Code's hold-SPACE dictation needs a microphone the container does not have, which is what the macOS-only audio bridge existed for; macOS dictates into any focused window with the microphone key, terminal sessions included), and a Troubleshooting entry answers the same question directly. Two sections moved at the user's request: the web interface up to just after the two demo GIFs, and the v3 section down to just above Troubleshooting, renamed "Migrating from v3".
