...
- **FR-25** (stage 1 - totopo): The advanced demo GIF is re-recorded, since it currently stars the audio server. Both GIFs are also brought to v4.0.0. (The plan listed this as a host step; it turned out to render inside the container - the demos are synthetic and the renderer, fonts and Pillow were already provisioned there - so it was done during phase 4 instead.) Totopo's README keeps its current structure, minus the voice sections (FR-08).
...
- **FR-27** (stage 1 - totopo): The README carries a "Migrating from v3" section (just above Troubleshooting) for a v3.16 user: a v3.16 workspace just works - the first v4 run tidies the dead audio settings itself (FR-30) - and the section says what changes on that first run (voice is gone; everything else behaves as in v3.16). Only older shapes need a step: run totopo v3.16.0 once (`npx totopo@3.16.0`) to bring the workspace to the v3.16 shape - that run performs any previously needed migration - then return to v4.
