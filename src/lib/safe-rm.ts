...
const TOTOPO_HOME = resolve(join(homedir(), ".totopo"));
const TEST_TMP_PREFIX = join(tmpdir(), "totopo-test-");
...
export function safeRmSync(path: string, options?: RmOptions): void {
    ...
    const ok = r === TOTOPO_HOME || r.startsWith(TOTOPO_HOME + sep) || basename(r) === "totopo.yaml" || r.startsWith(TEST_TMP_PREFIX);
