// =========================================================================================================================================
// src/lib/select-tools.ts - multiselect UI for choosing container runtimes
// =========================================================================================================================================

import { cancel, isCancel, log, multiselect } from "@clack/prompts";
import type { HostRuntimes } from "./detect-host.js";

const dimText = (s: string) => `\x1b[2m${s}\x1b[0m`;
...
export async function selectTools(hostRuntimes: HostRuntimes): Promise<string[]> {
    ...
    log.success(`Node.js  ${dimText(`${nodeVersion} · always included`)}`);

    const options = OPTIONAL_TOOLS.map(({ key, label }) => {
        const detected = hostRuntimes[key];
        return {
            value: key as string,
            label: detected ? `${label}  ${dimText(`v${detected} · host`)}` : `${label}  ${dimText("latest")}`,
        };
    });
    ...
    const selected = await multiselect({
        message: `Select additional tools:\n   ${dimText("Space to toggle · Enter to confirm")}`,
        options,
        initialValues,
        required: false,
    });
